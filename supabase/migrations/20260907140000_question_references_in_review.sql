-- ============================================================================
-- Expõe question_references/sources no payload que o estudante já recebe ao
-- responder/revisar uma questão (submit_question_attempt / get_question_review).
--
-- Contexto: question_references e sources já existem desde
-- 20260904140000_schema_v2_conteudo_real.sql, mas nenhum caminho de leitura
-- (RPC nem SELECT direto) as expunha. Esta migration só ACRESCENTA um campo
-- 'references' ao jsonb já retornado por essas duas funções — não muda
-- nenhuma coluna, tabela ou a lógica de validação/gravação existente.
--
-- Formato de cada item de 'references' (nulo/ausente quando a questão não
-- tem nenhum vínculo estruturado — não inventa dado): source_id, citation_text,
-- tipo, verificacao, identificadores (jsonb livre — doi/pmid/url/isbn/etc.,
-- usado no frontend só para montar um link quando existir um identificador
-- reconhecível, nunca inventado).
-- ============================================================================
